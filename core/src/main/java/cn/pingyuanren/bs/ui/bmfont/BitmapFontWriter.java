/*
 * Vendored from libGDX gdx-tools (com.badlogic.gdx.tools.bmfont.BitmapFontWriter),
 * Apache License 2.0. 仅依赖 libGDX 核心（Pixmap/PixmapIO/BitmapFontData/Glyph/PixmapPacker），
 * 无 AWT / LWJGL，可安全编入 TeaVM。
 *
 * 用途：把 FreeType 生成的 BitmapFont 烘焙成 .fnt + .png（BMFont 文本格式），
 * 运行时直接 new BitmapFont(.fnt) 加载，无需 FreeType / TTF / freetype.js。
 */
package cn.pingyuanren.bs.ui.bmfont;
...
public class BitmapFontWriter {
	 ...
	 *
	 * The FontInfo parameter is useful for cleaner output; such as including a size and font face name hint. However, it can
	 * be null to use default values. libGDX ignores most of the "info" line when reading back fonts, only padding is used. Padding
