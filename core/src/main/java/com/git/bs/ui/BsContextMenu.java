...
///
/// 实现要点：
/// - 沿用 `BsMenuPopup` 的 backdrop 方案：透明全屏 actor 捕获"点外部"/Esc 关闭。
/// - item 用 `TextButton`（style = `bs-menu-item`），事件路径清晰。
/// - 定位在点击坐标，做边界纠正（下方/右侧不够则翻转）；弹层延后一帧（postRunnable），
///   避开"在 touchDown 中 addActor 被同一事件移除"的坑。
/// - 触屏无右键 → 用 `Timer` 实现长按；移动超过阈值取消。
///
/// v1 不含：子菜单、键盘上下方向键导航、图标项。可后续扩展。
/// @author authorZhao
/// @since 2026-07-16
@Slf4j
public class BsContextMenu {
    ...
    /// skin 参数保留以对齐其它组件 API；实际渲染始终取当前主题 skin（见 `buildAndShow`），
    ...
    /// 会把 actor 置为 `Touchable.enabled`（否则接收不到事件）；重复 attach 同一 actor 会先移除旧监听。
