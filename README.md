...
核心组件（90+）、三主题（Light / Dark / Admin）、FreeType 中文字体、Skin 加载/导出、跨平台启动器均已落地，可以拿来构建真实界面。仍有打磨空间（性能、字符集瘦身、Web 端适配、文档），欢迎试用与反馈。💪

## ✨ 特性

### 🧩 组件生态

- **90+ Bootstrap 风格组件** 🏗️：Button / Form / InputGroup / SelectBox / Table / DataTable / Pagination / Navbar / MenuBar / Breadcrumb / Card / Modal / Window / Toast / Alert / Dialog / Tooltip / Popover / Tabs / Accordion / Collapse / Offcanvas / Drawer / Carousel / Slider / Progress / Spinner / Badge / Tag / Avatar / Tree / Steps / Timeline / Transfer / ColorPicker / DatePicker / FileItem …
- **9 种图表** 📈：Line / Bar / Area / Pie / Doughnut / Scatter / Spline / Radar（基于 Scene2D 自绘，无第三方图表库，支持 hover tooltip / 图例交互）
- **4 种对话框** 💬：Alert / Confirm / Prompt / Choice

### 🎨 主题与皮肤

- **Light / Dark / Admin 三主题** 🌗：纯代码生成主题色 + Drawable，运行时一键切换，支持自定义主题
- 🚧 **卡通皮肤（规划中）**：面向游戏 UI 的卡通描边/大圆角/活泼配色风格皮肤，让组件库直接服务游戏内界面

### ⚙️ 工程能力

- **FreeType 中文字体** 🔤：开箱即用，支持分帧预热、字号分级、按需字符集生成
- **Skin 工具链** 🛠️：`BsSkinLoader`（json + atlas + FreeType 加载）、`BsSkinExporter`（导出供 Skin Composer 二次编辑）、`BsIconPackager`（SVG → atlas，desktop 端）
- **国际化** 🌍：`BsI18n`，zh_cn / en_us / ja_jp，properties 翻译包，core 与业务翻译分层
- **资源加密** 🔐：单 `assets.pak` + ChaCha20 运行时解密（`bs-ui-res`），详见 [docs/pak-consumer-guide.md](./docs/pak-consumer-guide.md)
- **VISUI 风格全局 API** 🎯：`BsUI.getSkin()` / `BsTheme.tp()`，组件无需自己持有 Skin

### 🖥️ 平台与 Native 支持

- **桌面** 🪟🐧：LWJGL3（Windows / Linux）
- **Web** 🌐：TeaVM（HTML5 / WebAssembly，浏览器直接跑）
- 🧪 **GraalVM Native Image**：`native-image` 编译成原生可执行文件（exe / elf），秒级启动、免装 JRE
- 🧪 **HTML → 单文件原生程序**：借助 web-dev 库把 Web 版产物连同 HTML 一起打进一个 `exe` / `elf`，分发即双击运行
...
|------|------|
| ☕ **Java** | **25+（必须）** —— 编译与运行都是 |
| 🧱 构建工具 | Gradle（仓库自带 wrapper，无需安装） |

> ⚠️ **重要**：本项目基于 **Java 25** 构建（虚拟增强特性 + 最新语法），而 Android 平台官方工具链远未支持到 Java 25，因此 **bs-ui 不支持安卓**，选型时请注意。

## 📱 平台支持矩阵

| 平台 | 状态 | 说明 |
|------|:----:|------|
| 🪟 Windows | ✅ | 主力测试平台，Releases 提供开箱即用的 jar |
| 🐧 Linux | ✅ | 源码运行已验证 |
| 🌐 Web（TeaVM） | ✅ | 基本测试通过 |
| 🖥️ Native（GraalVM） | 🧪 | 实验性，配置见 `lwjgl3/nativeimage.gradle` |
| 🍎 macOS | 🧪 | 理论可用，未验证 |
| 🤖 Android | ❌ | 不支持（Java 25+ 门槛） |
| 📲 iOS | ❌ | 不支持 |
...
- 🖥️ **桌面**（含全部资源，装了 Java 25 直接 `java -jar` 运行）：
...
> ⚠️ **Web 版提示**：首次打开需**下载约 20MB 的资源包 + wasm**，网速慢时首屏可能要等较长时间（白屏/加载页属于正常现象），请耐心等待或优先体验桌面版。
...
| `bs-res` | `bs-ui-res` | 🔐 pak 资源加密工具箱（BPK1 容器、ChaCha20、`PakBootstrap`） |
| `assets-skin` | `bs-assets-skin` | 🖼️ 纯资源：烘焙好的皮肤（atlas / png / json / 位图字体） |
...
| `lwjgl3` | — | 🖥️ Desktop 启动器 + 平台实现 + iconpkg 打包工具（依赖 Batik）+ GraalVM native 配置 |
| `teavm` | — | 🌐 Web 启动器 |

## 🗺️ Roadmap

- 🎨 **卡通皮肤**：面向游戏 UI 的卡通风格主题（描边、大圆角、活泼配色）
- 🧪 **Native 发布链路**：GraalVM native-image 稳定化 + web-dev 库打包 HTML 到单文件 `exe` / `elf`
- ⚡ 性能优化与字符集瘦身
- 📖 文档与示例补全
...
- **[Bootstrap](https://getbootstrap.com/)** —— 设计语言与组件形态的来源 🅱️
- **[Bootstrap Icons](https://icons.getbootstrap.com/)** —— 图标体系
- **[noto-emoji](https://github.com/googlefonts/noto-emoji.git)** —— emoji 资源 😀
- **[霞鹜文楷 / LXGW WenKai](https://github.com/lxgw/LxgwWenKai)** —— 内置中文字体，清新易读 ✒️
...
- **[Lombok](https://projectlombok.org/) / [SLF4J](http://www.slf4j.org/) / [Fastjson2](https://github.com/alibaba/fastjson2)** —— 开发基础设施 🛠️
